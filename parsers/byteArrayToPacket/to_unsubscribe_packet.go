...
func ToUnsubscribePacket(byteArray *[]byte, connectionID int64) (packets.Packet, errorinfo.GatewayError) {
	...
	if key, err := tries.ConvertStringToSubscriptionKey(rawKey); err == nil {
		return dto.CreateUnsubscribePacket(key, packetId, connectionID), nil
	} else {
		return nil, err
	}
}
